...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'criar' || $action === 'editar') {
        ...
        if (empty($titulo) || empty($autor)) {
            ...
            if ($action === 'criar') {
                $stmt = $pdo->prepare("INSERT INTO livros (titulo, autor, categoria, sinopse, quantidade, ano_publicacao, capa_url) VALUES (?,?,?,?,?,?,?)");
                $stmt->execute([$titulo, $autor, $categoria, $sinopse, $qtd, $ano, $capa]);
                $msg = 'Livro cadastrado com sucesso!';
            } else {
                $eid = (int)($_POST['id'] ?? 0);
                $stmt = $pdo->prepare("UPDATE livros SET titulo=?, autor=?, categoria=?, sinopse=?, quantidade=?, ano_publicacao=?, capa_url=? WHERE id=?");
                $stmt->execute([$titulo, $autor, $categoria, $sinopse, $qtd, $ano, $capa, $eid]);
        ...
        <div class="card form-card">
            ...
            <form method="POST" class="grid-form">
                ...
                <div class="field-group">
                    ...
                    <input type="number" name="ano_publicacao" min="1000" max="2099" value="<?= h($editando['ano_publicacao'] ?? '') ?>">
                </div>
                <div class="field-group">
                    <label>URL da Capa</label>
                    <input type="url" name="capa_url" placeholder="https://..." value="<?= h($editando['capa_url'] ?? '') ?>">
                </div>
        ...
        <div class="card" style="margin-top:2rem">
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;margin-bottom:1.5rem">
                ...
                <form method="GET" style="display:flex;gap:.5rem">
                    <input type="text" name="busca" placeholder="Buscar..." value="<?= h($busca) ?>" style="padding:.5rem 1rem;border:1px solid var(--border);border-radius:8px;background:var(--bg);color:var(--text)">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <?php if ($l['capa_url']): ?>
                                    <img src="<?= h($l['capa_url']) ?>" alt="" style="width:40px;height:56px;object-fit:cover;border-radius:4px">
                                <?php else: ?>
                            ...
                            <td>
                                <div style="display:flex;gap:.5rem">
                                    ...
                                    <form method="POST" style="display:inline" onsubmit="return confirm('Excluir este livro?')">
